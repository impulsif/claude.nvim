...
function M.setup(user_config)
    -- Use vim.tbl_extend instead of tbl_deep_merge
    if user_config then
    ...
    require("plenary.curl").post({
        ...
        callback = vim.schedule_wrap(function(response)
            if response.status ~= 200 then
                vim.notify(string.format("[claude.nvim] HTTP Error %d: %s", response.status, response.body), vim.log.levels.ERROR)
                return
            end
...
function M.define_commands()
    ...
    api.nvim_create_user_command("ClaludeConfigure", function(opts)
        ...
        local config = {}
        if opts.fargs[1] then config.model = opts.fargs[1] end
        if opts.fargs[2] then config.temperature = tonumber(opts.fargs[2]) end
        M.setup(config)
    end, { nargs = "*" })
end
